...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
        ...
        <div class="h-48 w-full bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 relative"></div>
        ...
        <div class="px-6 pb-6 relative flex flex-col sm:flex-row sm:items-end justify-between -mt-16 gap-6">
            <div class="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                ...
                <div class="mb-2">
                    <div class="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
                        ...
                        <span class="bg-blue-50 text-blue-700 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-blue-100 uppercase">
                            {{ __('Mahasiswa') }}
                        </span>
                    </div>
            ...
            @if($isOwnProfile)
                <div class="flex justify-center sm:justify-end gap-3 self-center sm:self-end">
                    <a 
                        href="{{ route('profile') }}" 
                        class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors shadow-sm shadow-blue-100"
                    >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-2 space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                <div class="relative border-l border-slate-100 ml-4 space-y-6">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <h2 class="text-md font-bold text-slate-900 mb-4 flex items-center gap-2">
                    <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex flex-wrap gap-2">
                    @forelse($user->skills as $skill)
                        <span class="text-xs bg-slate-50 text-slate-700 border border-slate-200/60 px-3 py-1.5 rounded-lg font-medium shadow-sm flex items-center gap-1">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                <div class="divide-y divide-slate-100 text-sm">
                    <div class="py-3 flex justify-between">
                        ...
                        <span class="font-bold text-slate-900">{{ $user->studentProfile->student_id ?? '-' }}</span>
                    </div>
                    <div class="py-3 flex justify-between">
                        <span class="text-slate-400 font-medium">Program Studi</span>
                        <span class="font-bold text-slate-900 text-right max-w-[160px] truncate" title="{{ $user->studentProfile->study_program ?? '-' }}">
                            {{ $user->studentProfile->study_program ?? '-' }}
                        </span>
                    </div>
                    <div class="py-3 flex justify-between">
                        <span class="text-slate-400 font-medium">Fakultas</span>
                        <span class="font-bold text-slate-900 text-right max-w-[160px] truncate" title="{{ $user->studentProfile->faculty ?? '-' }}">
                            {{ $user->studentProfile->faculty ?? '-' }}
                        </span>
                    </div>
                    <div class="py-3 flex justify-between">
                        <span class="text-slate-400 font-medium">Angkatan</span>
                        <span class="font-bold text-slate-900">{{ $user->studentProfile->entry_year ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
